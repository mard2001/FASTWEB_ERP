...
<html lang="en">
    <head>
        ...
        <title>Counter Receipt - {{ $customer->Name ?? 'Customer' }}</title>
    ...
    <body>
        ...
        @php
            ...
            // Ensure at least one page renders even when there are no pending transactions
            if ($totalPages === 0) {
        ...
        @foreach($chunks as $pageIndex => $pageData)
            ...
            <header class="px-2 py-1">
                ...
                <div class="d-flex justify-content-between">
                    <p class="text-nowrap text-center statement-header my-0" style="font-size: 16px;">Counter Receipt</p>
                </div>
                ...
                <div class="d-flex justify-content-between headerDetails">
                    <div>
                        <p class="mb-0" style="font-size: 12px;">Report Type: <strong>Counter Receipt (Pending Transactions)</strong></p>
                ...
                <tbody>
                    ...
                    @php
                        ...
                        // Determine paid display
                         ...
                         if ($isPayment) {
                             ...
                             if (isset($row['paid']) && $row['paid'] != 0) {
                                 if ($row['paid'] < 0) {
                                     // Show negative amount in parentheses like customer credit table
                    ...
                    <tr style="{{ $rowStyle }}">
                        ...
                        <td class="text-center">
                            ...
                            @else
                                <span style="background-color: #ffc107; color: white; padding: 2px 6px; border-radius: 3px; font-size: 8px;">Pending</span>
                            @endif
                        </td>
                            ...
                            <tbody>
                                @if($pendingTransactions->count() > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding-left:20px; font-size: 12px; font-weight: bold;">₱{{ number_format($pendingTransactions->where('type', 'payment')->sum('paid'), 2) }}</td>
                                    </tr>
                                    <tr>
                                        <th style="font-size: 12px;">Total Balance Due:</th>
                                        <td style="padding-left:20px; border-bottom: 3px double #000; font-size: 12px; font-weight: bold;">₱{{ number_format($pendingTransactions->sum('amount') - $pendingTransactions->where('type', 'payment')->sum('paid'), 2) }}</td>
                                    </tr>
                                @else
                                    <tr>
                                        <th style="font-size: 14px;">STATUS:</th>
                                        <td style="padding-left:20px; border-bottom: 3px double #000; font-size: 14px; font-weight: bold; color: #28a745;">NO PENDING TRANSACTIONS</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
    ...
    <script>
        ...
        function exportToExcel() {
            ...
            ws_data.push(['COUNTER RECEIPT']);
            ws_data.push([]);
            ws_data.push(['Customer:', customerName]);
            ws_data.push(['Customer Code:', customerCode]);
            ws_data.push(['Contact Number:', customerContact]);
            ws_data.push([]);
            ...
            // Add table headers
            ws_data.push(['Date', 'Description', 'SO No', 'Amount', 'Paid', 'Balance', 'Status', 'Terms']);
            ...
            visibleRows.forEach(row => {
                ...
                if (cells.length >= 8) {
                    const rowData = [
                        ...
                        cells[2].textContent.trim(), // SO No
            ...
            if (!ws[titleCellRef]) ws[titleCellRef] = { t: 's', v: 'COUNTER RECEIPT' };
            ws[titleCellRef].s = {
            ...
            const filename = `Customer_Counter_Receipt_${customerName.replace(/[^a-zA-Z0-9]/g, '_')}_${currentDate}.xlsx`;
