...
<header class="px-2 py-1">
    ...
    <div class="d-flex justify-content-between po-info">
        ...
        <div class="d-flex">
            ...
            <div class="px-2">
                <p class="m-0"> {{ $po->PODate }} </p>
                <p class="m-0"> {{ $po->PONumber }} </p>
            </div>
        </div>
    </div>
</header>
...
<body>
    <div class="d-flex justify-content-between flex-wrap my-2">
        <div class="my-sm-2 bg-info bg-opacity-25 po-info" style="width: 40%;">
            ...
            <div class="bg-info py-1 px-2 d-flex align-items-center bg-opacity-50">
                {{ $po->posupplier['SupplierName'] }}
            </div>

            <div class="d-flex px-2">
                <span>Contact: </span>
                <p class="mx-2 my-0"> {{ $po->posupplier['ContactPerson'] }}
                </p>
            </div>

            <div class="px-2">
                <p class="text-wrap m-0 pe-3">
                    {{ $po->posupplier['CompleteAddress'] }}
                </p>

                <div>
                    {{ $po->posupplier['ContactNo'] }}
                </div>
            </div>
        </div>
        ...
        <div class="my-sm-2 bg-info bg-opacity-25 po-info" style="width: 40%;">
            ...
            <div class="d-flex px-2">
                <span>Contact: </span>
                <p class="mx-2 my-0"> {{ $po->contactPerson }}
                </p>
            </div>
    ...
    <table class="table table-bordered my-2 po-info">
        <tbody>
            ...
            <tr>
                <td class="col p-1">{{ $po->EncoderID }}</td>
                <td class="col p-1">Road Delivery</td>
                <td class="col p-1">{{ $po->FOB }}</td>
                <td class="col p-1"> {{ $po->posupplier['TermsCode'] }}
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <table class="table table-bordered po-info">
        ...
        <tbody class="border-dark">
            ...
            <tr>
                <td class="col py-1">{{ $item['StockCode'] }}</td>
                <td class="col py-1">{{ $item['Decription'] }}</td>
                <td class="col-1 py-1 text-center"> {{ $item['UOM']}}</td>
                <td class="col py-1 text-end">{{ number_format($item['Quantity'], 1)}}</td>
                <td class="col py-1 text-end"> {{ number_format($item['PricePerUnit'], 2) }}</td>
                <td class="col py-1 text-end"> {{ number_format($item['TotalPrice'], 2) }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td class="col py-1 text-end"> {{ number_format($po->subTotal - ($po->subTotal * .12), 2) }}</td>
            </tr>

            <tr>
                <td rowspan="3" colspan="4" class="col">
                    <p>{{ $po->SpecialInstruction }}</p>
                </td>
                <td class="col py-1">TAX</td>
                <td class="col py-1 text-end"> {{ number_format($po->subTotal * .12, 2)}}</td>
            ...
            <tr>
                ...
                <td class="col py-1 text-end"> {{ count($po->POItems) }}</td>

            </tr>

            <tr>
                <td class="col py-1">TOTAL</td>
                <td class="col py-1 text-end"> {{ number_format($po->totalCost, 2)}}</td>
            </tr>
        </tfoot>
    </table>
</body>
