...
var jsonArr = [];
var selectedGcash = null;
var isloading = false;

$(document).ready(async function () {
    // Load gcash reconciliation data (all gcash accounts auto-populated)
    await datatables.loadGcashReconData();
    ...
    $("#gcashReconTable").on("click", ".addBalanceBtn", async function (e) {
        e.stopPropagation(); // Prevent row click event
        const gcashId = $(this).data('gcash-id');
        const accountName = $(this).data('account-name');

        openBeginningBalanceModal(gcashId, accountName);
    });

    // Handle row click to show details
    $("#gcashReconTable").on("click", "tbody tr", async function (e) {
        // Don't trigger if clicking on button
        if ($(e.target).closest('.addBalanceBtn').length) {
            return;
        }

        const gcashId = $(this).attr('data-gcash-id');
        ...
        // Load and show gcash details with transaction history
        await showGcashDetails(gcashId);
    });
    ...
    $("#saveBeginningBalanceBtn").on("click", async function () {
        if (validateBeginningBalance()) {
            const data = {
                GcashID: parseInt($('#BalanceGcashID').val()),
    ...
    $("#manualDepositBtn").on("click", function () {
        ...
        openManualTransactionModal(gcashId, accountName, 'IN');
    ...
    $("#manualWithdrawBtn").on("click", function () {
        ...
        openManualTransactionModal(gcashId, accountName, 'OUT');
        // Don't hide the gcash details modal
    });
    ...
    $("#saveManualTransactionBtn").on("click", async function () {
        if (validateManualTransaction()) {
            ...
            const data = {
                GcashID: parseInt($('#ManualGcashID').val()),
...
function openBeginningBalanceModal(gcashId, accountName, existingData = null) {
    $('#BalanceGcashID').val(gcashId);
...
function openManualTransactionModal(gcashId, accountName, transactionType) {
    $('#ManualGcashID').val(gcashId);
    ...
    // Don't hide gcash details modal - keep it in background
...
async function saveManualTransaction(data) {
    await ajax('api/gcash-reconciliation/manual-transaction', 'POST', JSON.stringify({ data: data }), (response) => {
        if (response.success) {
            ...
            Swal.fire({
                ...
                // Reload gcash details to show new transaction
                showGcashDetails(data.GcashID);
...
async function saveBeginningBalance(data) {
    await ajax('api/gcash-reconciliation/set-beginning-balance', 'POST', JSON.stringify({ data: data }), (response) => {
        if (response.success) {
...
async function showGcashDetails(gcashId) {
    $("#gcashReconTable tbody").css('pointer-events', 'none');

    await ajax('api/gcash-reconciliation/' + gcashId, 'GET', null, (response) => {
        if (response.success) {
            const gcash = response.data;
            selectedGcash = gcash;

            // Fill gcash information (using .val() for input fields)
            $('#DetailsGcashID').val(gcash.GcashID);
            $('#detailsAccountName').val(gcash.AccountName);
            $('#detailsAccountNumber').val(gcash.AccountNumber);

            // Fill reconciliation summary (using .val() for input fields)
            $('#summaryBeginningBalance').val('₱' + formatNumber(gcash.BeginningBalance || 0));
            $('#summaryTotalInflows').val('₱' + formatNumber(gcash.TotalInflows || 0));
            $('#summaryTotalOutflows').val('₱' + formatNumber(gcash.TotalOutflows || 0));
            $('#summaryAvailableBalance').val('₱' + formatNumber(gcash.AvailableBalance || 0));

            // Load transaction history
            loadTransactionHistory(gcash.transactions || []);

            $('#gcashDetailsModal').modal('show');
        } else {
            Swal.fire({
                title: "Error",
                text: response.message,
        ...
        $("#gcashReconTable tbody").css('pointer-events', 'auto');
    }, (xhr, status, error) => {
        $("#gcashReconTable tbody").css('pointer-events', 'auto');
        if (xhr.responseJSON && xhr.responseJSON.message) {
            Swal.fire({
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    // For Gcash payments, show the payment type
                    const lowerData = data.toLowerCase();
                    if (lowerData === 'gcash' || lowerData.includes('gcash')) {
                        return 'Gcash';
                    }
...
const datatables = {
    loadGcashReconData: async () => {
        await ajax('api/gcash-reconciliation', 'GET', null, (response) => {
            jsonArr = response.data;
            console.log('Gcash reconciliation data loaded:', response.data);
            datatables.initGcashReconDatatable(response);
            if(isloading){
                Swal.close();
                isloading = false;
            }
        }, (xhr, status, error) => {
            console.error('Error loading gcash reconciliation data:', error);
            Swal.fire({
                title: "Error",
                text: "Failed to load gcash reconciliation data.",
                icon: "error"
            });
        });
    },
    initGcashReconDatatable: (response) => {
        if (response.success) {
            if (MainTH) {
                MainTH.clear().draw();
                MainTH.rows.add(response.data).draw();
            } else {
                MainTH = $('#gcashReconTable').DataTable({
                    data: response.data,
                    language: {
                    ...
                    createdRow: function (row, data) {
                        $(row).attr('data-gcash-id', data.GcashID);
                    ...
                    initComplete: function () {
                        ...
                        tableDiv.after('<div style="background: linear-gradient(to right, var(--primary-color, #1b438f), var(--secondary-color, #33336F) ); color: var(--text-color-light, #FFF); margin-top:10px; padding: 10px 15px; border-top-left-radius:10px; border-top-right-radius: 10px;"><p style="margin:0px">Gcash Reconciliation - All Accounts</p></div>');
                    }
                });
            }
        }
...
function downloadToCSV(jsonArr) {
    ...
    link.download = `GcashReconciliation_${today}.csv`;
